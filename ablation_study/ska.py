    ...
    for dataset_ in datasets_:
        for approach in approaches:
            for backbone in backbones:
                ...
                train_dataloader, _, datasets, exp_run, label_inf = get_dataset(
                    config, exp=exp
                )
                best_states_file = os.path.join(
                    exp_run.paths["states"], "val_track.txt"
                )
                best_states = []
                ...
                agent = get_agent(
                    config, model=model, label_names=label_inf["label_names"]
                )
                ...
                X_old_diffusion = (
                    X_old_diffusion.detach()
                    .numpy()
                    .reshape(X_old_diffusion.shape[0], -1)
                )
                X_old_injection = (
                    X_old_injection.detach()
                    .numpy()
                    .reshape(X_old_injection.shape[0], -1)
                )
                ...
                X_new_diffusion = (
                    X_new_diffusion.detach()
                    .numpy()
                    .reshape(X_new_diffusion.shape[0], -1)
                )
                X_new_injection = (
                    X_new_injection.detach()
                    .numpy()
                    .reshape(X_new_injection.shape[0], -1)
                )
                ...
                df = pd.DataFrame(dict_)
                df.to_csv(
                    os.path.join(out_dir, "old_knowledge_diffusion.csv"), index=False
                )
                ...
                df = pd.DataFrame(dict_)
                df.to_csv(
                    os.path.join(out_dir, "old_knowledge_injection.csv"), index=False
                )
