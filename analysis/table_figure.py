...
if __name__ == "__main__":
    ...
    for name in os.listdir(file_dirs):
        if dataset in name:
            ...
            if not os.path.exists(
                os.path.join(
                    file_dirs, name, "0", "results", "result_{}.csv".format(metric)
                )
            ):
                continue
            result = extracted_test(
                os.path.join(
                    file_dirs, name, "0", "results", "result_{}.csv".format(metric)
                )
            )
            ...
            for i in range(len(up_bound[0])):
                ravg_curve.append(
                    np.average(np.array(result[i, : i + 1] / up_bound[0, : i + 1]))
                )
    ...
    for key in table_dict:
        print(
            key.rjust(16),
            str(round(table_dict[key][0], 3)).rjust(16),
            str(round(table_dict[key][1], 3)).rjust(16),
            str(round(table_dict[key][2], 3)).rjust(16),
            str(round(table_dict[key][3], 3)).rjust(16),
        )

    dataframe = pd.DataFrame(table_dict)
    dataframe.to_csv(
        os.path.join(out_dir, dataset + "_results.csv"), index=False, sep=","
    )

    plt.title("avg_curve")
    for key in table_dict:
        plt.plot(
            [i for i in range(len(table_dict[key][4]))], table_dict[key][4], label=key
        )
    plt.legend()
    ...
    for key in table_dict:
        plt.plot(
            [i for i in range(len(table_dict[key][5]))], table_dict[key][5], label=key
        )
    plt.legend()
    ...
    for key in table_dict:
        plt.plot(
            [i for i in range(len(table_dict[key][6]))], table_dict[key][6], label=key
        )
    plt.legend()
    ...
    for key in table_dict:
        plt.plot(
            [i for i in range(len(table_dict[key][7]))], table_dict[key][7], label=key
        )
    plt.legend()
