...
class UnbiasedKnowledgeDistillationLoss(nn.Module):
    ...
    def forward(self, inputs, targets, mask=None):
        ...
        if (inputs.size()[
                1] - new_cl) > 1:  # Ensure that the outputs_no_bgk is not empty if new_cl is 1 and inputs.size()[1] is 1 eg.
...
class LossPLOP(nn.Module):
    ...
    def forward(self, x, x_o, y):
        ...
        y = copy.deepcopy(y)
        foreground_label = (1. - torch.argmax(x_o, dim=1)) + y[:, 0, :, :]
        foreground_label = torch.where(foreground_label > 0., torch.ones([1]).to(x.device),
