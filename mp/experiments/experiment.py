...
from mp.paths import storage_path, abs_path
...
class Experiment:
    ...
    def __init__(self, config=None, name='', notes='', reload_exp=False):
        ...
        self.path = os.path.join(os.path.join(abs_path, storage_path, 'exp'), self.name)
        # Restore files
        ...
        try:
            ...
            if isinstance(data, Data):
                ...
                self.splits = split_dataset(data, test_ratio=self.config.get('test_ratio', 0.0),
                                            val_ratio=self.config['val_ratio'], nr_repetitions=1,
                                            cross_validation=False)
...
class ExperimentRun:
    ...
    def save_state(self, state_name, pytorch_dict={}, np_dict={}, pkl_dict={}):
        ...
        if 'optimizer' in pytorch_dict:
            ptlr.save_optimizer_state(pytorch_dict['optimizer'], name=state_name + '_optimizer',
                                      path=self.paths['states'])
        if 'scheduler' in pytorch_dict:
            ptlr.save_scheduler_state(pytorch_dict['scheduler'], name=state_name + '_scheduler',
                                      path=self.paths['states'])
    ...
    def restore_state(self, state_name, pytorch_dict, np_dict, pkl_dict, device='cuda:0'):
        success = True
        if 'model' in pytorch_dict:
            success = success and ptlr.load_model_state(pytorch_dict['model'], name=state_name + '_model',
                                                        path=self.paths['states'], device=device)
        if 'optimizer' in pytorch_dict:
            success = success and ptlr.load_optimizer_state(pytorch_dict['optimizer'], name=state_name + '_optimizer',
                                                            path=self.paths['states'], device=device)
        if 'scheduler' in pytorch_dict:
            success = success and ptlr.load_scheduler_state(pytorch_dict['scheduler'], name=state_name + '_scheduler',
                                                            path=self.paths['states'], device=device)
