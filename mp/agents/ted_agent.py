...
from mp.eval.losses.losses_segmentation import LossAKT


class TEDAgent(SegmentationAgent):
    ...
    def train(
        ...
        for epoch in range(init_epoch, nr_epochs):
            ...
            acc = self.perform_training_epoch(
                ...
                loss_akt=LossAKT(gugf=config["gugf"]),
            )
            if val_best:
    ...
    def perform_training_epoch(
        ...
        for data in tqdm(train_dataloader, disable=True):
            ...
            if self.model.unet_old != None:
                ...
                outputs_new_noise = self.get_outputs(noise)
                outputs_old_noise = self.get_outputs_simple(noise)
                loss_distill_noise = self.multi_class_cross_entropy_no_softmax(
                    outputs_new_noise, outputs_old_noise
                )

                # akt loss
                loss_distill_akt = loss_akt(outputs, outputs_old)

                loss_distill = (
                    loss_distill_noise + loss_distill_image + loss_distill_akt
                )
            else:
                if loss_seg.is_cuda:
