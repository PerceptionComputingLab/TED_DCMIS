...
from mp.eval.losses.losses_distillation import UnbiasedKnowledgeDistillationLoss
import torch
...
class MIBAgent(SegmentationAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mib_loss = UnbiasedKnowledgeDistillationLoss()
    ...
    def perform_training_epoch(self, loss_f, train_dataloader, config, epoch,
        ...
        for data in tqdm(train_dataloader, disable=True):
            ...
            if self.model.unet_old != None:
                ...
                loss_mib = self.mib_loss(outputs, outputs_old)
            else:
                if loss_seg.is_cuda:
                    loss_mib = torch.zeros(1).to(loss_seg.get_device())
                else:
                    loss_mib = torch.zeros(1)

            loss = loss_seg + config['lambda_d'] * loss_mib
            loss.backward()
            ...
            acc.add('loss_mib', float(loss_mib.detach().cpu()), count=len(inputs))

        # self.model.unet_scheduler.step()

        if print_run_loss:
            print('\nrunning loss: {} - mib {} - time/epoch {}'.format(acc.mean('loss'), acc.mean('loss_mib'),
