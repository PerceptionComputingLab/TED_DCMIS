...
from mp.utils.pytorch.pytorch_load_restore import save_model_state, load_model_state, save_optimizer_state, \
...
class Agent:
    ...
    def perform_training_epoch(self, optimizer, loss_f, train_dataloader,
                               print_run_loss=False, *args, **kwargs):
    ...
    def train(self, results, optimizer, loss_f, train_dataloader,
              ...
              save_path=None, save_interval=10, *args, **kwargs):
                ...
                for index, value in enumerate(eval_dict.items()):
                    if index == 5:  # foreground
                        with open(os.path.join(save_path, 'val_log.txt'), 'a+') as f:
                            f.writelines(f"{str(epoch)} running loss: {acc.mean('loss')}")
