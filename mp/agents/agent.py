...
class Agent:
    ...
    def perform_training_epoch(
        self, optimizer, loss_f, train_dataloader, print_run_loss=False, *args, **kwargs
    ):
    ...
    def train(
        ...
        **kwargs,
        ...
        for epoch in range(init_epoch, init_epoch + nr_epochs):
            ...
            self.perform_training_epoch(
                optimizer, loss_f, train_dataloader, print_run_loss=print_run_loss
            )
    ...
    def restore_state(self, states_path, epoch, optimizer=None):
        ...
        state_full_path = os.path.join(states_path, state_name)
        try:
            correct_load = load_model_state(
                self.model, "model", state_full_path, device=self.device
            )
            assert correct_load
            ...
            if optimizer is not None:
                load_optimizer_state(
                    optimizer, "optimizer", state_full_path, device=self.device
                )
            if self.verbose:
    ...
    def track_validation_metrics(self, idx, loss_f, datasets, save_path, epoch, acc):
        i = 0
        for ds_name, ds in datasets.items():
            if "val" == ds_name[1] and i == idx:
                eval_dict = ds_losses_metrics(ds, self, loss_f, ["ScoreDice"])
                for index, value in enumerate(eval_dict.items()):
                    if index == 5:  # foreground
                        with open(os.path.join(save_path, "val_log.txt"), "a+") as f:
                            f.writelines(
                                f"{str(epoch)} running loss: {acc.mean('loss')}"
                            )
                            f.writelines(" val:" + str(value[1]["mean"]) + "\n")
                        return value[1]["mean"]
            elif ds_name[1] == "test":
                i = i + 1
