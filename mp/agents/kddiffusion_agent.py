...
class KDDiffusionAgent(SegmentationAgent):
    ...
    def ablation(self, train_dataloader, dataset_list, config, dataset_index):
        ...
        pseudo_data = SimpleDataset(
            subset=dataset_list[dataset_index],
            size=(config["input_dim_hw"], config["input_dim_hw"]),
        )
        pseudo_data_loader = DataLoader(
            pseudo_data, batch_size=config["batch_size"], shuffle=True
        )

        features = {
            "outputs": [],
            "outputs_old": [],
            "outputs_new_pseudo_img": [],
            "outputs_old_pseudo_img": [],
            "outputs_new_injection_img": [],
            "outputs_old_injection_img": [],
            "outputs_new_ska_img": [],
            "outputs_old_ska_img": [],
            "outputs_new_aug_img": [],
            "outputs_old_aug_img": [],
        ...
        with torch.no_grad():
            for data in tqdm(train_dataloader, disable=True):
                ...
                features["outputs_new_pseudo_img"].append(
                    outputs_new_pseudo_img.to("cpu")
                )

                outputs_old_pseudo_img = self.get_old_bottom_block(pseudo_img)
                features["outputs_old_pseudo_img"].append(
                    outputs_old_pseudo_img.to("cpu")
                )
                # noise add
                noise_img = torch.randn_like(inputs).to(inputs.device) + inputs
                outputs_new_noise_img = self.get_new_bottom_block(noise_img)
                features["outputs_new_injection_img"].append(
                    outputs_new_noise_img.to("cpu")
                )
                outputs_old_noise_img = self.get_old_bottom_block(noise_img)
                features["outputs_old_injection_img"].append(
                    outputs_old_noise_img.to("cpu")
                )
    ...
    def perform_training_epoch(
        ...
        for data in tqdm(train_dataloader, disable=True):
            ...
            if self.model.unet_old != None:
                ...
                loss_distill_image = self.multi_class_cross_entropy_no_softmax(
                    outputs, outputs_old
                )
                ...
                pseudo_img = next(iter(pseudo_data_loader))
                try:
                    pseudo_img = pseudo_img.to(
                        inputs.device
                    )  # + torch.randn_like(pseudo_img).to(
                ...
                loss_distill_noise = self.multi_class_cross_entropy_no_softmax(
