...
def get_mask_labels(instances):
    ...
    for instance in instances:
        ...
        assert all(
            x == int(x) for x in instance_labels
        ), "Mask contain non-integer values"
...
def check_correct_nr_labels(labels, instances):
    ...
    print(nr_labels)
    assert nr_labels <= len(
        labels
    ), "There are mask indexes not accounted for in the manually supplied label list"
