...
class SegmentationInstance(Instance):
    def __init__(
        self, x_path, y_path, name=None, class_ix=0, group_id=None, target_class=-1
    ):
        ...
        target_class: select one class when the number of target is more than one
        ...
        if target_class != -1:
            trans = torchio.Lambda(lambda x: torch.where(x == target_class, 1.0, 0.0))
            y = trans(y)

        self.shape = x.shape
        super().__init__(x=x, y=y, name=name, class_ix=class_ix, group_id=group_id)

    def get_subject(self):
        return torchio.Subject(x=self.x, y=self.y)
