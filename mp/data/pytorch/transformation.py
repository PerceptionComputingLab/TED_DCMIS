...
NORMALIZATION_STRATEGIES = {
    None: None,
    "rescaling": torchio.transforms.RescaleIntensity(
        out_min_max=(0, 1), percentiles=(0.1, 99.0)
    ),
...
AUGMENTATION_STRATEGIES = {
    ...
    "standard": torchio.transforms.Compose(
        [
            ...
            torchio.transforms.RandomFlip(p=0.1, axes=(1, 0, 0)),
            torchio.transforms.RandomMotion(
                p=0.1, degrees=10, translation=10, num_transforms=2
            ),
            torchio.transforms.RandomBiasField(p=0.1, coefficients=(0.5, 0.5), order=3),
            torchio.transforms.RandomNoise(p=0.1, mean=(0, 0), std=(50, 50)),
            torchio.transforms.RandomBlur(p=0.1, std=(0, 1)),
        ]
    ),
