...
def _get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="experiment name for new or resume",
    )

    # hardware
    parser.add_argument(
        "--device", type=str, default="cuda", help="device type cpu or cuda"
    )
    parser.add_argument(
        "--device-ids", nargs="+", default=[5], type=int, help="ID(s) of GPU device(s)"
    )
    parser.add_argument(
        "--n-workers",
        type=int,
        default=2,
        help="# multiplied by # of GPU to get # of total workers",
    )

    # dataset
    parser.add_argument(
        "--dataset", type=str, default="prostate", help="datasets to training"
    )
    parser.add_argument(
        "--target_class",
        type=str,
        default="i",
        help="cardiac target. i:LV-endo, o:LV-epi, r:RV",
    )
    parser.add_argument(
        "--test-ratio",
        type=float,
    ...
    parser.add_argument(
        "--input_dim_c", type=int, default=1, help="input channels for images"
    )
    parser.add_argument(
        "--input_dim_hw", type=int, default=192, help="height and width for images"
    )
    parser.add_argument(
        "--no-resize",
        action="store_true",
    ...
    parser.add_argument(
        "--approach", type=str, default="kd", help="which approach to train"
    )
    parser.add_argument(
        "--backbone", type=str, default="unet", help="backbone:unet or swinunet"
    )
    parser.add_argument(
        "--val-best",
        action="store_true",
        default=True,
        help="best validation to next domain",
    )
    ...
    parser.add_argument(
        "--display-interval", type=int, default=1, help="display/tensorboard interval"
    )
    parser.add_argument(
        "--run-loss-print-interval", type=int, default=1, help="run_loss_print_interval"
    )

    # loss weighting
    parser.add_argument(
        "--loss-type", type=str, default="dice_bce", help="which type of loss to train"
    )
    parser.add_argument(
        "--lambda-d",
        type=float,
        default=0.001,
        help="lambda forc tuning MAS or KD loss",
    )

    # noise level on ska
    parser.add_argument("--mean", type=float, default=0, help="lambda for tuning mean")
    parser.add_argument("--var", type=float, default=1, help="lambda for tuning var")

    # U-Net
    parser.add_argument(
        "--unet-dropout", type=float, default=0, help="apply dropout to UNet"
    )
    parser.add_argument(
        "--unet-monte-carlo-dropout",
        type=float,
        default=0,
        help="apply monte carlo dropout to UNet",
    )
    parser.add_argument(
        "--unet-preactivation",
        action="store_true",
...
def parse_args(argv):
    ...
    args.device = str(
        args.device + ":" + str(args.device_ids[0])
