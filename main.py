...
for idx, dataloader in enumerate(train_dataloader):
    ...
    agent.train(results=results, loss_f=loss_f, train_dataloader=dataloader, test_dataloader=dataloader, config=config,
                init_epoch=init_epoch, nr_epochs=nr_epochs, eval_datasets=datasets, save_path=exp_run.paths['states'],
                dataset_index=idx, exp_path=exp_run.paths['states'])
    if config['approach'] not in ['seq']:
        config['continual'] = True
if config['dataset'] == 'mm':
    config['dataset'] = 'cardiac'
exp_run.finish(results=results, plot_metrics=['Mean_ScoreDice[{}]'.format(config['dataset'])])
