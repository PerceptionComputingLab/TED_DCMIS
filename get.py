...
def get_dataset(config, exp):
    ...
    if config["approach"] in ["joint"]:
        joint_dataset = torch.utils.data.ConcatDataset(
            datasets[(name, "train")] for name in subset_list
        )
        ...
        joint_dataset = torch.utils.data.ConcatDataset(
            datasets[(name, "test")] for name in subset_list
        )
...
def get_loss_type(config):
    if config["loss_type"] == "dice":
        ...
        loss_g = LossDiceBCE(bce_weight=1.0, smooth=1.0, device=config["device"])
...
def get_agent(config, model, label_names):
    ...
    agent = agents[config["approach"]](
        model=model, label_names=label_names, device=config["device"]
    )
    return agent
